...
export default function BrotesMicrogreens({products}){
  ...
  const {allproducts, setAllproducts, openSidebar, setOpenSidebar} = useContext(StoreContext)
  ...
  useEffect(() => {
    ...
    setPageProducts(allproducts.filter(product => {
      return product.category === 'Brotes - Microgreens'
    }))
  ...
  return(
    ...
    <div className='flex w-full flex-col gap-4 my-6 px-2 md:px-3 lg:px-10 min-h-screen'>
      <section id='descripcion' className='flex flex-col justify-center items-center gap-4 w-full'>
        <h1 className='font-text font-semibold text-[20px] lg:text-[22px] text-center w-full'>Brotes - Microgreens</h1>
        <p className='font-text text-justify text-[16px] md:text-[18px] lg:text-[20px]'>
          Son pequeñas versiones de vegetales y hierbas que ofrecen un sabor intenso y una textura crujiente, populares en la comida gourmet debido a su aspecto visual atractivo.
        </p>
      </section>
